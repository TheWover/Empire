...
.SYNOPSIS
    ...
    Execute a .NET Assembly from memory in the current process.

    PowerSploit Function: Invoke-Assembly
...
.DESCRIPTION
    ...
    Invoke-Assembly loads and executes a .NET Assembly from memory in the context of the current process using the .NET API call System.Reflection.Assembly.Load. An entry point may be specified. If no entry point is specified and the .NET Assembly has a default entry point, then that default will be invoked. Arguments may be provided in the form of command-line parameters.
    
    Mixed Assemblies cannot be loaded from memory.

.PARAMETER Assembly64

    Base64-encoded .NET Assembly.
    
.PARAMETER Class

    (Optional) Class of the entry point. It must be marked as public. A namespace may be specified in the format of "Namespace.Class".
    
.PARAMETER Method

    (Optional) Method of the entry point. The method must be marked as public and static.

.PARAMETER Arguments

    (Optional) Command-line parameters to pass in to the Assembly.


.EXAMPLE

    C:\PS> Invoke-Assembly -Assembly64 <base64-encoded .NET Assembly>

    Description
    -----------
    Loads and executes a .NET Assembly from memory. Uses the default entry point and does not provide any command-line parameters.
    This may only be performed for EXEs, since DLLs do not possess default entry points.

.EXAMPLE

    C:\PS> Invoke-Assembly -Class Seatbelt.Program -Method Main -Arguments "system BasiOSInfo" -Assembly64 <base64-encoded .NET Assembly>

    Description
    -----------
    Loads and executes a .NET Assembly from memory. Specifies an entry point and executes it with parameters.
